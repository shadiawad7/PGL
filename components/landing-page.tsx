    ...
    <main className="min-h-screen overflow-x-clip bg-background text-foreground">
      <section ref={heroRef} className="pgl-hero relative bg-primary text-primary-foreground">
        <header className="absolute inset-x-0 top-0 z-20 mx-auto flex max-w-7xl items-center justify-between px-6 py-7 lg:px-10">
          ...
          <nav className="hidden items-center gap-5 text-xs font-medium uppercase tracking-[0.16em] md:flex" aria-label="Navegación principal"><SessionNavigation />
        ...
        {menuOpen && <nav className="absolute inset-x-0 top-20 z-30 flex flex-col gap-5 border-y border-primary-foreground/15 bg-primary px-6 py-6 text-sm uppercase tracking-[0.16em] md:hidden"><SessionNavigation /><a href="#metodo" onClick={goToMethod}>Método</a><Link href="#entrenamientos" onClick={() => setMenuOpen(false)}>Entrenamientos</Link><Link href="#contacto" onClick={() => setMenuOpen(false)}>Contacto</Link><Link className="text-accent" href="/reservar">Reservar sesión</Link></nav>}
        <div id="inicio" className="relative mx-auto grid max-w-7xl items-start gap-6 px-6 pb-12 pt-36 lg:grid-cols-[1.05fr_0.95fr] lg:gap-0 lg:px-10 lg:pb-20 lg:pt-44">
          <div className="relative z-10 lg:pr-8 lg:pt-12">
            ...
            <div className="mt-14 max-w-md border-t border-white/15 pt-6 lg:mt-20">
          ...
          <div className="pgl-hero-photo relative aspect-[2/3] lg:-ml-12 lg:w-[calc(100%+3rem)]">
            ...
            <p className="absolute bottom-6 right-3 font-mono text-[10px] uppercase tracking-[0.22em] text-white/50">PGL / En movimiento</p>
          </div>
        </div>
      </section>
