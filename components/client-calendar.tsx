...
export function ClientCalendar({ reservations }: { reservations: Reservation[] }) {
  ...
  return <section className="rounded-2xl border border-border bg-background/85 p-3 backdrop-blur-md sm:p-8">
    ...
    <div className="mt-6 grid grid-cols-7 border-l border-t border-border">{weekdayNames.map((day) => <div key={day} className="border-b border-r border-border bg-secondary px-2 py-3 text-center font-mono text-xs font-bold">{day}</div>)}{Array.from({ length: start }).map((_, index) => <div key={`empty-${index}`} className="min-h-20 sm:min-h-28 border-b border-r border-border bg-secondary/50" />)}{Array.from({ length: days }).map((_, index) => { const day = index + 1; const key = new Date(month.getFullYear(), month.getMonth(), day).toISOString().slice(0, 10); const reservation = reservedDates.get(key); return <div key={key} className={`min-h-20 sm:min-h-28 border-b border-r border-border min-w-0 p-1 sm:p-2 ${key === today ? 'bg-accent/10' : 'bg-background'}`}><span className={`inline-flex size-7 items-center justify-center font-mono text-xs font-bold ${reservation ? 'bg-accent text-accent-foreground' : ''}`}>{day}</span>{reservation && <div className="mt-2 text-[10px] sm:text-xs font-semibold leading-5"><p>{new Intl.DateTimeFormat('es-ES', { hour: '2-digit', minute: '2-digit' }).format(new Date(reservation.startsAt))}</p><p className="truncate text-muted-foreground">{reservation.training}</p></div>}</div> })}</div>
