...
export default async function TrainingDetail({ params }: { params: Promise<{ training: string }> }) {
  ...
  return <main className="reservation-page min-h-screen"><header className="flex items-center justify-between border-b border-white/15 bg-black/25 px-6 py-6 lg:px-10"><Link href="/reservar" className="font-mono text-sm font-bold tracking-[0.18em]">PGL<span className="text-lime-300">.</span>TRAINNING</Link><div className="flex items-center gap-5"><Link href="/mi-cuenta" className="text-sm font-semibold">Mi cuenta</Link><Link href="/" className="text-sm font-semibold">Inicio</Link></div></header><div className="mx-auto max-w-6xl px-6 py-16 lg:px-10"><Link href="/reservar" className="font-mono text-xs uppercase tracking-[0.16em] text-lime-300">← Volver a formatos</Link><h1 className="mt-10 text-5xl font-bold tracking-[-0.06em] sm:text-7xl">{detail.name}</h1><p className="mt-6 max-w-xl text-lg leading-8 text-white/75">{detail.description}</p><div className="mt-12 flex flex-col gap-4 border-t border-white/20 pt-7 sm:flex-row sm:items-center sm:justify-between"><p className="font-mono text-xs uppercase tracking-[0.14em]">{detail.durationMinutes} minutos · Godella, Valencia</p><p className="text-sm font-semibold">Calle San Blas 15</p></div><div className="reservation-calendar"><AvailabilityCalendar training={detail.id} /></div></div></main>
}
